...
Templer lets you embed scripts right in your templates. This is inspired by, but a little different than what you see in vue.js single file templates.

There are two kinds of Templer scripts. Generate scripts for generating your site, and runtime scripts for running it.

## Site Generation Scripts

When you want to create html pages based on data, you can write javascript which Templer runs as it builds your site. These are called generate scripts.

Templer treats generate scripts like they are the body of a [javascript promise](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise). You probably don't need to know about promises in detail to use Templer. Just understand that you can do what ever you want asynchronously, and when you are done, call 'resolve' or 'reject' with an error. Find out more details in the [generate script](/templates/generateScript/) documentation.
...
When Templer processes the template above, it will strip out the generate script so that it won't get rendered to the final html file(s). It will instead run the script to build up data to pass to any variables or logic flow you have in your ejs template.

#### Pre and Post Generate Scripts

Templer honours two special scripts, one that is guaranteed to run before anything else, and one that runs after your site build is complete. The pre generation script is a great place to call and cache api responses, build up a table of contents, etc. The post generation script is a good place to summarize the results of your build, likely writing log information or to a json
...
Templer makes it quite easy to add javascript to your application while at the same time integrating nicely with modern and awesome tools like [vite](https://vitejs.dev/) and any javascript framework you can think of.
...
Templer allows you to add an entry script to a template like so:
...
The above template is being rendered directly to root, because the generate path is '/'. This is a special case in which Templer will copy the entry script you specify into a file called "main.js" alongside index.html in the root of your [output directory](/guide/setup/).
...
If you're using something like [alpine.js](https://alpinejs.dev/), you might want to organize your code to build up reusable components. Templer makes this easy with the lib script feature.
...
Obviously we wouldn't want this code to be written multiple times a component includes it, so instead templer writes it to your output in parallel to the folder structure you had it in in your templates.
